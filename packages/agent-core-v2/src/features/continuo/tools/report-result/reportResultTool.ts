    ...
    return {
      ...
      execute: async () => {
        ...
        await this.store.update(doc.workspaceId, (current) => ({
          ...
          activity: [...current.activity, { at: reportedAt, taskId: task.taskId, kind: 'task', text: `Result reported: ${args.deliverables.length} deliverable(s)${(args.unresolved ?? []).length > 0 ? `, ${(args.unresolved ?? []).length} unresolved` : ''}` }],
        }));
