...
Name the task first: `name` is two to six characters in the language of the folder, and `category` is a lowercase ASCII word taken from the project's own category list when it has one. The product files the work log as `work-log-<date>-<category>-<name>.md` and shows `name` wherever this task appears, so a vague name makes the record unreadable.
