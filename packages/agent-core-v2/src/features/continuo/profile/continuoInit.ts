...
const INIT_ROLE =
  ...
  '- Then follow those guides to a small number of materials that show what the folder is for and how its files are named and written. Stop as soon as you can state what the folder is for, where the key materials live and the conventions in use; do not keep reading to double-check. Skip anything the guides mark as archive, plus dependencies, build output, caches, version-control internals and anything that looks like credentials. For non-text files note the name only; never try to open them with text tools.\n' +
