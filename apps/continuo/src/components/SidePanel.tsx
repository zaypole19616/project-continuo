import { FolderOpen, History, Layers, Loader2 } from 'lucide-react';
...
export function SidePanel({ mode, onMode, workspaceId, root, doc, target, onNavigate, onSelectTask, onPatchContext, onReunderstand, onError, searchRef }: {
      ...
      <aside>
        <header className="pane-header chrome" style={{ padding: '0 8px 0 12px' }}>
          <TabsList>
            ...
            <TabsTrigger value="context"><Layers />记住的事{doc?.init.status === 'running' ? <Loader2 className="size-3 animate-spin text-accent" /> : pending > 0 && <span className="mode-badge">{pending}</span>}</TabsTrigger>
        ...
        <TabsContent value="context" className="pane-body p-4">{doc ? <ContextPanel doc={doc} onPatch={onPatchContext} onOpenFile={(p) => onNavigate({ kind: 'file', path: p })} onReunderstand={onReunderstand} /> : <Loading />}</TabsContent>
