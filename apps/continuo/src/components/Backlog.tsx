...
import type { ContinuoDoc, ContinuoTodo, TodoTiming } from '#/lib/api';
import { localDay, localTime, taskLabel } from '#/lib/trajectory';
import { Button } from '#/components/ui/button';
import { Hint } from './PlanList';
...
function metaOf(doc: ContinuoDoc, todo: ContinuoTodo): string {
  if (todo.fromTaskId !== undefined) {
    ...
    return `建议的下一步${from === undefined ? '' : ` · 来自「${taskLabel(from)}」`}`;
  }
...
export function Backlog({ doc, busy, lockReason, onAdd, onStart, onDelete }: {
  doc: ContinuoDoc; busy: boolean; lockReason: string | undefined;
  onAdd: (text: string, timing: TodoTiming | undefined) => Promise<boolean>; onStart: (todo: ContinuoTodo) => void; onDelete: (todo: ContinuoTodo) => void;
}) {
      ...
      {todos.map((todo) => (
        <div key={todo.todoId} className="todo-row">
          ...
          <div className="flex shrink-0 gap-1">
            <Hint title={lockReason}><Button variant="ghost" size="sm" disabled={busy} onClick={() => onStart(todo)}><Play size={12} />开始</Button></Hint>
            <button className="icon-btn" title="删除这件待办" aria-label="删除这件待办" onClick={() => onDelete(todo)}><Trash2 size={14} /></button>
          </div>
        </div>
      ))}
