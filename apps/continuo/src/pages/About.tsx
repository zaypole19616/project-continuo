...
const BETS: Bet[] = [
  {
    ...
    inProduct: ['产物出现在文件夹网格里，带「产物」标记，点它跳到产出它的任务。', '来自 README 的约定直接生效，Agent 的推断只是候选，等你确认。', '每个工作空间有一份工作日志，看板与日志读的是同一份账本。'],
  },
  {
  ...
  },
  {
    ...
    points: [
      ...
      { h: '理解有来源', p: 'Agent 的每条理解标注来自哪个文件，分"生效"和"候选"。' },
      { h: '完成由外部证明', p: '"完成"不是模型说的，是交付物真的在、未解决项如实列出。' },
    ],
    inProduct: ['Context 面板里每条条目带来源、状态和修订，可确认、纠正、停用。', '交付核验合并 Agent 上报与实际写入，缺的进「待复核」而不是假装完成。', '源文件变了，依赖它的条目自动标成"来源已变化"。'],
  },
  {
    ...
    inProduct: ['决定类条目排在注入的最前面，它就是这个工作空间的价值函数。', '每次纠正都留下"纠正前、纠正后、当时的任务"，这是训练方向判断的真实数据。', '两处替模型补课的兜底都带退役条件：模型做到了，兜底就退。'],
  },
];
  ...
  return (
    <div className="about">
      <nav className="about-nav chrome">
        ...
        <button className="btn btn-primary btn-sm" onClick={onOpenApp}>打开工作空间</button>
      </nav>
      {bet === null ? (
        <>
