...
打开产品是一张启动卡：左边「创建项目」、右边「打开已有项目」，下面是最近打开的项目（只有名字和路径）。第一次启动先弹一个三页的功能介绍（只有"下一步"），左边是一两句话，右边是产品本身：第一页是 Continuo 的做事方式，三个判断排成一列，选中哪条，右边的镜头就移到产品里对应的地方（4 秒自动轮换）；第二页是一张真的决策卡，可以左右划；第三页是两条轨迹并排，点「切换」或树里的箭头，选中的那条高亮。启动卡上的「功能介绍」可以随时重看。
...
| | 内容 |
|---|---|
| 对话 | 当前这条轨迹的会话。每件事做完，收尾卡跟在那一轮后面：做出的文件、没做到的、读过的资料；模型认为还有值得做的下一步时，下面是「建议的下一步事项」（✓ 立即执行 / ✗ 划掉 / ≡+ 加入待办），处理之前什么都不会开始。决策以一张卡出现在它发生的位置 |
...
| 轨迹 | 这个项目的树：根节点是打开文件夹时的「了解这个文件夹」（读过什么、理解、要点，失败时可以重新了解）；主干是当前轨迹上的事项和决策，别的方案和另起的轨迹作为分叉挂在它们离开主干的地方。右上角两个视图：概览（每个节点一行：名字加产物或状态，决策下列出其余方案）和详情（每个节点展开成 背景 / 任务 / 过程 / 结果，决策展开成同一叠方案卡）；概览里点一个节点，只展开那一个。采用或切换方案、从这里继续、放弃都在这里做 |
...
- 轨迹导出只有接口，没有界面入口。
- 「生成更多方案」和「自定义方向」只在决策还没选的时候出现；「放弃」只在选定之后出现在其他方案上。
