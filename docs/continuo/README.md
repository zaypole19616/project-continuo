...
主窗口三栏：左栏导航（新任务、当前工作空间及其任务记录、其他工作空间、账户），中栏对话（居中 760；空状态是理解摘要加一行计数，下一步就是输入框），右栏可收起的面板：「文件」是文件树 + 文件夹网格 + 预览（产物可以和这次任务动手前的那一版对比，来自 Kimi Code 的 turn 级 file history），「记住的事」是账本，「工作记录」按日期、按事项列出每个会话的工作日志：时间、原始要求、后来的补充、读过的资料、这次记住的、结果、产出、未完成、建议的下一步。界面只说人话；三个判断的名字只在首次启动的引导里出现一次。没有单独的首页，也没有单独的打开文件夹页：左边是对话列表，中间是对话，和 Codex 一样。没选中对话时中间是居中的输入框，输入框上标着当前文件夹；点它打开唯一的文件夹菜单——最近的文件夹、打开已有的文件夹、新建文件夹、用演示文件夹试试（服务端在 `~/Continuo Demo/` 生成一份合成材料并注册为工作空间）。侧栏顶部的文件夹行打开的是同一个菜单。第一次启动先弹一个四步的新手引导（只有"下一步"），结束直接进产品。了解这个文件夹是产品自己的事，不占一条会话：进度和结果都在右侧「记住的事」里（读取时是一行阶段提示，读完是理解摘要加「重新了解」）。中栏永远是对话——了解期间直接交代任务，中栏立刻变成那条会话。视觉直接用 Kimi Code 自己的设计变量：官方 web UI 以预构建包的形式随仓库发布（`apps/kimi-code/dist-web`），`apps/continuo/src/theme.css` 的颜色、圆角、阴影、侧栏宽度 264px、面板宽度 460px、输入框圆角 32px、发送按钮都取自它的 `assets/index-*.css`；布局也照它：侧栏是 新会话 / 搜索 / 会话（文件夹分组、会话挂在下面），新会话页是字标 + 文件夹 chip + 输入框。
