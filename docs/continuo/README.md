...
**交代任务：带着有效 context 干活。** 每个任务开一个独立会话，Harness 在每一步把有效 context 编成一段不超过 6000 字的参考数据注入（决定 > 约定 > 进度 > 背景 > 材料），压缩之后重新注入。任务结束时 Harness 核验交付物：Agent 用 `ReportWorkspaceResult` 上报路径，同时 Harness 自己记录了它实际写过哪些文件，两边合并后逐个检查文件是否存在，才决定是"完成"还是"还差一点"。完成后自动追加一条"进度"条目，下次任务就知道这件事做过了。每个任务结束都有一张收尾卡：做出的文件、没做到的、读过哪些资料、这次记住的；模型认为还有一件值得做的事时，收尾卡后面多一张「接下来，也许值得做这一步」，写清依据，点了才会开新任务。

**卡点、纠正、重开。** Agent 需要决定时提 AskUserQuestion 或等审批，任务进"需要你"列；如果它只是用一句话问了你，Harness 把任务标成"等你回复"而不是"完成"，你的回复送回同一个会话续接。你可以在右侧「记住的事」里确认、改一下、不再用任何一条：纠正会生成一条新条目替代旧条目（origin 变为"你确认的"），下一个任务立刻按新约定行事。关掉再打开：不会重跑初始化，正在跑的任务被标为中断可续；如果某条 context 依赖的源文件在两次打开之间变了，这条会被标成"来源变了，先不用"，等你说"仍然有效"或让 Agent 重新看。

演示文件夹（`~/Continuo Demo/`）只是一份合成材料，和你自己的文件夹走同一条路径：打开后它开始了解，了解期间你可以浏览文件，也可以直接交代任务。理解卡上的"重新了解"会用真实的只读理解任务重跑一遍。

### 界面：对话为主，文件夹在右

主窗口三栏：左栏导航（新任务、当前工作空间及其任务记录、其他工作空间、账户），中栏对话（居中 760；空状态是理解摘要加一行计数，下一步就是输入框；理解卡、提问卡、审批卡、收尾卡、下一步卡带小标签标明体现的判断），右栏可收起的面板：「文件」是文件树 + 文件夹网格 + 预览（产物可以和这次任务动手前的那一版对比，来自 Kimi Code 的 turn 级 file history），「记住的事」是账本，「工作记录」按日期列出这个文件夹里发生过的事。界面只说人话，Orderliness / Proactiveness / Clarity 这些名字只在介绍页出现。没有单独的首页，也没有单独的打开文件夹页：左边是对话列表，中间是对话，和 Codex 一样。没选中对话时中间是居中的输入框，输入框上标着当前文件夹；点它打开唯一的文件夹菜单——最近的文件夹、打开已有的文件夹、新建文件夹、用演示文件夹试试（服务端在 `~/Continuo Demo/` 生成一份合成材料并注册为工作空间）。侧栏顶部的文件夹行打开的是同一个菜单。第一次启动先弹一个四步的新手引导（只有"下一步"），结束直接进产品。打开自己的文件夹后中栏只显示"正在了解你现有的工作…"这类阶段提示，不列文件数和读取步骤，理解好了再展示理解卡。视觉直接用 Kimi Code 自己的设计变量：官方 web UI 以预构建包的形式随仓库发布（`apps/kimi-code/dist-web`），`apps/continuo/src/theme.css` 的颜色、圆角、阴影、侧栏宽度 264px、面板宽度 460px、输入框圆角 32px、发送按钮都取自它的 `assets/index-*.css`；布局也照它：侧栏是 新会话 / 搜索 / 会话（文件夹分组、会话挂在下面），新会话页是字标 + 文件夹 chip + 输入框。`#/about` 是四个判断的介绍页（Orderliness / Proactiveness / Clarity / Direction）。
...
| 对话 + 文件 + 记住的事 | `apps/continuo/`（Vite + React，独立于官方 web UI） | 显性层只做三件事：需要你时才打断、打断带上下文、随时可看可接管 |
