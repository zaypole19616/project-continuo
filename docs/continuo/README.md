...
打开产品是一张启动卡：左边「创建项目」、右边「打开已有项目」，下面是最近打开的项目（只有名字和路径）。第一次启动先弹一个三页引导（只有"下一步"），左边是一两句话，右边是产品本身：第一页是 Continuo 的做事方式，三个判断排成一列，选中哪条，右边的镜头就移到产品里对应的地方（4 秒自动轮换）；第二页是一张真的决策卡，可以左右划；第三页是两条轨迹并排，点「切换」或树里的箭头，选中的那条高亮。启动卡上的「使用引导」可以随时重看。
