...
演示文件夹自带一段历史（`seedDemoDoc`）：一次已完成的理解、一份 9 月 15 日的初稿、一条口头定下的定价决定和一条"改稿另存"的规矩。点"用演示文件夹开始体验"直接进入"第二次打开"，没有等待；第一张体验卡"接着上次的初稿，把定价那节改成上周定的方案"跑完，收尾卡上是"用上了 7 条记住的事，追问 0 次"。自带历史的任务没有会话记录，界面会写明；理解卡上的"重新了解"会用真实的只读理解任务重跑一遍。打开自己的文件夹时不预置任何东西。

### 界面：对话为主，文件夹在右

主窗口三栏：左栏导航（新任务、当前工作空间及其任务记录、其他工作空间、账户），中栏对话（居中 760；空状态是三张体验卡，各对应一个判断，点一下预填任务；理解卡、提问卡、审批卡、产物卡带小标签标明体现的判断），右栏可收起的面板，「文件」模式是文件树 + 文件夹网格 + 预览，「记住的事」模式是账本。界面只说人话，Legibility / Proactiveness / Clarity 这些名字只在介绍页出现。没有单独的首页：打开就是产品本身，第一次启动先弹一个四步的新手引导（只有"下一步"），结束直接进产品；没选文件夹时中栏是两个入口——新建文件夹、打开已有的文件夹——下面一行小字可以用演示文件夹（服务端在 `~/Continuo Demo/` 生成一份合成材料、注册为工作空间并写入自带历史）。打开自己的文件夹后中栏只显示"正在了解你现有的工作…"这类阶段提示，不列文件数和读取步骤，理解好了再展示理解卡。视觉照 Kimi 的产品：暖白画布、黑白加一个 Kimi 蓝、Schibsted Grotesk、大圆角。`#/about` 是四个判断的介绍页。
