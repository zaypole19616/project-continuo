...
**打开项目：先读懂，不动手。** 一个只读 profile 的 Agent 扫描目录，先读 AGENTS.md / README 这类指引文件（如果 `work-log/` 里已有记录，也读最近几份），再看目录结构：目录结构已经能说明这个文件夹在做什么、材料在哪、怎么命名，就不再打开别的文件；不够时才抽样几份材料，然后用一次 `WorkspaceContext` 调用记下两样东西：一段**这个文件夹是什么**（两三句：做什么用的、输入在哪、产物去哪、什么是归档），以及若干条**项目要点**（一句一条：命名约定、已经定下的决定、某类材料在哪）。每条要点都必须指向它来自哪个文件；说不出来源的不记。了解期间你可以浏览文件，也可以直接交代事情。
...
- 文件夹太大时不复制：打开时的扫描被截断，或扫描到的文件合计超过 300MB，轨迹就不另建目录，退回共用项目目录（这时拦截只保护其他轨迹上报过的产物）。
