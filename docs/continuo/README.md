...
主窗口沿用桌面工作台的三栏模型。左栏是导航：新任务、工作空间列表（每个打开过的文件夹）、任务记录（像聊天记录一样按时间排列，带状态点）、底部账户与模型。中栏是文件工作区：面包屑、搜索框、网格 / 列表切换、排序；文件夹用图标网格展示，文件旁标出「指引」和「产物」，点产物能跳到产出它的那次任务；点开文件在中栏预览（Markdown 渲染）。右栏是 Agent 面板：标题栏一排图标切换对话 / 看板 / Context / 日志，消息流里工具步骤折叠成一条活动时间线，输入框带模型标识和当前文件夹芯片，区分「新任务」和「回复 / 追问」。三栏共用一块中性材质，用分隔线区分；颜色只留给状态和文件类型；左右两栏都能收起，亮暗主题跟随系统。这套结构参考了 Kuse Desktop 的公开产品文档与界面，代码全部独立实现。
