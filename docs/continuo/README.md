...
**打开项目：先读懂，不动手。** 一个只读 profile 的 Agent 扫描目录，先读 AGENTS.md / README 这类指引文件（如果 `work-log/` 里已有记录，也读最近几份），再看目录结构：目录结构已经能说明这个文件夹在做什么、材料在哪、怎么命名，就不再打开别的文件；不够时才抽样几份材料，然后用一次 `WorkspaceContext` 调用记下两样东西：一段**这个文件夹是什么**（两三句：做什么用的、输入在哪、产物去哪、什么是归档），以及若干条**项目要点**（一句一条：命名约定、已经定下的决定、某类材料在哪）。每条要点都必须指向它来自哪个文件；说不出来源的不记。了解期间你可以浏览文件，也可以直接交代事情。 只在第一次打开时了解；以后从「最近打开」再进来不会重读，了解失败或被打断时，卡片上有「重新了解」。了解的过程是对话顶部的一张卡片，实时显示正在做什么、已经读了哪些文件（点一下就在左侧打开）。结束后这张卡片变成「接下来想做什么？」：模型从读到的内容里看出值得做或改进的地方（最多三条，每条带依据），就列出来，点「就做这个」直接开始；没有就等你说。交代了第一件事之后，卡片收成一行摘要。
...
| 事项 | 当前轨迹上还没做完的事，分三组：正在做（排队中、具体阶段、核对产物）、等你（选方案 / 回答 / 批准 / 回复，按钮分别是去选 / 去回答 / 去批准 / 去回复）、停下来（已暂停、被打断、没做成及原因、还差一点，可以继续或重试）。下面是「待办」：你加的以后要做的事，以及 Agent 收尾时建议的下一步；可以不定时、指定一个时间，或每天 / 每周重复，到点就在当前轨迹上自动开始。做完的事在对话里的收尾卡上 |
| 轨迹 | 这个项目的树：根节点是打开文件夹时的「了解这个文件夹」（读过什么、理解、要点，失败时可以重新了解）；主干是当前轨迹上的事项和决策点，别的方案和另起的轨迹作为分叉挂在它们离开主干的地方。可缩放：缩小只剩名字和状态，中间档多一行（产物或依据），放大后每个节点展开成 背景 / 任务 / 过程 / 结果；在任何一档点一个节点，只展开那一个。切换、走这条、从这里继续、放弃都在这里做 |
