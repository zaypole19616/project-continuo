...
That is the job this Agent is meant to do for knowledge work: the work should be **continuously held**, not restarted at every conversation. Materials, tasks and results live in one folder; what was learned and done stays in that folder; and the person can always see, and correct, the line it is playing. The folder and the person's judgment are the bass line that never stops; the lines of work that different plans lead to are the solos coming and going on top of it, and switching to another one never breaks the line underneath.

## 项目名说明

Continuo，通奏低音。巴洛克音乐里贯穿全曲、从不中断的低音声部，它在下面托住整首曲子，让上面的独奏来来去去。

这个 Agent 想做的就是这件事：让工作被持续接住，而不是每次对话都从头开始。资料、任务、成果放在同一个文件夹里；做过什么就留在这个文件夹里；人随时能看见它在演奏哪一条线，并且能纠正。文件夹和人的判断是那条不断的低音；不同方案走出的轨迹是上面来来去去的独奏，换一条，底下的线不断。
...
**方向不明时：给方案，由你选。** 如果材料定不了怎么做、而不同做法会得出不同的产物（比如 Q4 目标按哪个口径定），Agent 不替你挑，也不只问一句，而是开一个**决策点**：若干个真正不同的方案，每个写清依据和风险，完整内容落成项目里的方案文件（`work-log/plan-日期-分类-任务名-方案A.md`），然后停下来。每个方案都写一句「适合」什么情况；Agent 再表个态：卡组顶部总会写一句选哪个「取决于」什么——方案之间本来是不同方向，不硬排名次；只有材料里的事实能把选择定下来时，它才另外给一张标「建议」并写为什么。「不建议」只出现在确实有硬伤的方案上，必须写原因（分头写的方案交齐后，它先比较一次再表态）。对话里的决策卡是横向排开的一叠卡片，一张一个方案、露出下一张，左右滑动，建议的那张排第一；点方案卡上的箭头就采用它，按那个方案接着做。滑到最后一张是「其他方向」，两个同级的选项：「生成更多方案」（新方案追加进来，必要时重新表态，旧的不删；已经没有真正不同的方向时，会说明为什么并把需要你定的那个问题抛回来），以及「自定义方向」（点一下光标就落到下面的输入框，写你想要的方向）。
...
打开产品是一张启动卡：左边「创建项目」、右边「打开已有项目」，下面是最近打开的项目（只有名字和路径）。第一次启动先弹一个五步引导（只有"下一步"）：Continuo 是什么，然后三个判断各一步，最后是方案与轨迹；每一步标出在界面哪里能看到。启动卡上的「使用引导」可以随时重看。
...
| | 内容 |
|---|---|
| 对话 | 当前这条轨迹的会话。每件事做完，收尾卡跟在那一轮后面：做出的文件、没做到的、读过的资料；模型认为还有值得做的下一步时，下面是「建议的下一步事项」（✓ 立即执行 / ✗ 划掉 / ≡+ 加入待办），处理之前什么都不会开始。决策点以一张卡出现在它发生的位置 |
| 事项 | 当前轨迹上还没做完的事，分三组：正在做（排队中、具体阶段、核对产物）、等你（选方案 / 回答 / 批准 / 回复，按钮分别是去选 / 去回答 / 去批准 / 去回复）、停下来（已暂停、被打断、没做成及原因、还差一点，可以继续或重试）。下面是「待办」：你加的以后要做的事，以及 Agent 的建议（了解之后的建议、每件事收尾时建议的下一步）。建议标着「建议」，用 ✓ / ✗ / ≡+ 决定立即执行、划掉还是留作待办；可以不定时、指定一个时间，或每天 / 每周重复，到点就在当前轨迹上自动开始。做完的事在对话里的收尾卡上 |
| 轨迹 | 这个项目的树：根节点是打开文件夹时的「了解这个文件夹」（读过什么、理解、要点，失败时可以重新了解）；主干是当前轨迹上的事项和决策点，别的方案和另起的轨迹作为分叉挂在它们离开主干的地方。右上角两个视图：概览（每个节点一行：名字加产物或状态，决策点下列出其余方案）和详情（每个节点展开成 背景 / 任务 / 过程 / 结果，决策点展开成同一叠方案卡）；概览里点一个节点，只展开那一个。采用或切换方案、从这里继续、放弃都在这里做 |
...
| 决策点与轨迹 | 引擎侧 `Trajectory` 工具（`propose` / `expand` / `recommend` / `list`）与 `trajectory.ts`；服务侧 `choosePlan` / `expandPlans` / `abandonPlan` / `activateTrajectory` / `forkAfterTask` | 方向的判断留给人，而且判断的那一刻被完整记下：条件、候选及依据、选了哪个、执行到哪、放弃了哪个以及为什么。轨迹只追加不覆盖，每条都能单独读、单独切回去 |
...
- 轨迹导出只有接口，没有界面入口。
- 「生成更多方案」和「自定义方向」只在决策点还没选的时候出现；「放弃」只在选定之后出现在其他方案上。
